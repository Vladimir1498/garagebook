...
async def _verify_fleet_membership(fleet_id: UUID, user_id: UUID, db: AsyncSession) -> bool:
    """Check if user is a member of this fleet."""
    result = await db.execute(
        select(OrganizationMember).where(
            OrganizationMember.organization_id == fleet_id,
...
async def _verify_fleet_admin(fleet_id: UUID, user_id: UUID, db: AsyncSession) -> bool:
    """Check if user is owner or admin of this fleet."""
    result = await db.execute(
        select(OrganizationMember).where(
            OrganizationMember.organization_id == fleet_id,
            OrganizationMember.user_id == user_id,
...
async def get_fleet(fleet_id: UUID, user: User = Depends(get_current_user), sub=Depends(require_tier(SubscriptionTier.fleet)), db: AsyncSession = Depends(get_db)):
    if not await _verify_fleet_membership(fleet_id, user.id, db):
...
async def list_members(fleet_id: UUID, user: User = Depends(get_current_user), sub=Depends(require_tier(SubscriptionTier.fleet)), db: AsyncSession = Depends(get_db)):
    if not await _verify_fleet_membership(fleet_id, user.id, db):
        raise HTTPException(status_code=404, detail="Fleet not found")
...
async def invite_member(fleet_id: UUID, data: InviteRequest, user: User = Depends(get_current_user), sub=Depends(require_tier(SubscriptionTier.fleet)), db: AsyncSession = Depends(get_db)):
    if not await _verify_fleet_admin(fleet_id, user.id, db):
...
async def remove_member(fleet_id: UUID, member_id: UUID, user: User = Depends(get_current_user), sub=Depends(require_tier(SubscriptionTier.fleet)), db: AsyncSession = Depends(get_db)):
    if not await _verify_fleet_admin(fleet_id, user.id, db):
        raise HTTPException(status_code=403, detail="Only admins can remove members")

    result = await db.execute(select(OrganizationMember).where(OrganizationMember.id == member_id, OrganizationMember.organization_id == fleet_id))
