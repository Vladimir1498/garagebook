...
const CustomTooltip = ({ active, payload }: any) => {
  ...
  return (
    <div className="rounded-xl border border-surface-200 bg-white px-3 py-2 shadow-lg dark:border-surface-600 dark:bg-surface-800">
      ...
      <p className="text-sm font-semibold tabular-nums text-surface-900 dark:text-white">{total.toLocaleString('ru')} ₽</p>
    </div>
  )
}
      ...
      <div className="mt-2 flex flex-wrap justify-center gap-x-4 gap-y-1.5">
        {data.map((item, i) => (
          <div key={item.category} className="flex items-center gap-1.5 text-xs text-surface-600 dark:text-surface-400">
            ...
            <span className="font-semibold tabular-nums text-surface-800 dark:text-surface-200">{item.total.toLocaleString('ru')} ₽</span>
          </div>
        ))}
      </div>
    </div>
